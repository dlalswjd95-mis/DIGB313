		...
		function addSquare(){
			for(var i=0; i<50; i++){
				...
				allItems[i].className = 'smallBox rounded';
				...
				$(".smallBox").css("left", function(k){
					return k + Math.floor((Math.random()*400)); });
				$(".smallBox").css("top", function(k){
				...
				//마우스로 drag해서 작은네모 옮기기 
				$(".smallBox").on('mousedown',function(e){
				var node = $(this);
				var position = node.offset();
				var initialized = {
					x : position.left - e.pageX - 70,
					y : position.top - e.pageY - 110
				};
...
//					399.46875<br>414<br>312.234375<br>335<br>
					mouseup : function(e){
						$(this).off(handlers);
					}
				};
